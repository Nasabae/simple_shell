...
/**
 * ssa_find_env - find given environmental variable in linked list
...
/**
 * ssa_unsetenv - remove node in environmental linked list
 * @env: linked list
...
int ssa_unsetenv(list_t **env, char **str)
{
	...
	index = ssa_find_env(*env, str[1]); /* get idx of node to delete */
	free_double_ptr(str);
...
/**
 * ssa_setenv - create or modify existing environmental variable in linked list
 * @env: linked list
...
int ssa_setenv(list_t **env, char **str)
{
	...
	index = ssa_find_env(*env, str[1]); /* find idx to traverse to node */
	if (index == -1)
	{
